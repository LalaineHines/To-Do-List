...
export function renameList(oldListName, newListName) {
    const list = getAllLists();
    const foundList = allLists.find(lst => lst.name === oldListName);
    if (foundList) {
    ...
    const listItems = Array.from(document.querySelectorAll('#list-manager .list-items'));
