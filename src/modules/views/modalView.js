...
import { addListToListManager, renderListInput, renderTaskList, updateListTitle } from './listViews';
import { createToDoTask, updateTaskDetails } from '../models/taskModel';
import { getAllLists, renameLists } from '../models/listModel';
...
export function openDialog() {
    ...
    toDoForm.addEventListener('submit', (event) => {
        ...
        const priority = toDoForm.querySelector('.task-priority-input').value;
        const listName = toDoForm.querySelector('.task-list-input').value;
...
export function openDetailsDialog(task) {
    ...
    const dialog = document.createElement('dialog');
    dialog.classList.add('todo0dialog');
