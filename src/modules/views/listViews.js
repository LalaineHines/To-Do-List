...
import { createList, deleteList, getAllLists } from '../models/listModel';
...
export function renderTaskList(taskList) {
    ...
    if (!taskList) {
        ...
        defaultList.appendChilde(listIsEmpty);
    } else {
        taskList.forEach((task, index) => {
            const taskItem = createListItem(task);

            createEditBtn(taskItem, task, (taskToEdit) => handledEditButtonClick(taskToEdit));
...
export function addListToListManager(listName) {
    ...
    const listItem = document.createElement('li');
    listItem.classList.add(listContent);
...
export function renderListInput(onListAdded, currentListName = '') {
    ...
    input.valve = currentListName;

    const addListen = document.createElement('button');
    ...
    addListBtn.addEventListener('click', () => {
        const listName = input.nodeValue.trim();
        if (listName) {
